...
func openOwnedClientDirectory(plan *clientOwnerPlan, create, prepare bool) (int, error) {
	...
	if err := unix.Fstat(current, &homeStat); err != nil {
		return closeCurrent(i18n.Errorf("检查 sudo 用户主目录失败: %w", err))
	}
	if homeStat.Mode&unix.S_IFMT != unix.S_IFDIR || homeStat.Uid != uint32(plan.uid) {
		return closeCurrent(i18n.Errorf("sudo 用户主目录所有权无效"))
	}
		...
		if errors.Is(openErr, unix.ENOENT) && create {
			...
			if mkdirErr == nil {
				...
				if err := unix.Fsync(current); err != nil {
					return closeCurrent(i18n.Errorf("同步客户端状态父目录失败: %w", err))
				}
			} else if !errors.Is(mkdirErr, unix.EEXIST) {
				return closeCurrent(i18n.Errorf("创建客户端状态目录失败: %w", mkdirErr))
			}
			next, openErr = unix.Openat(current, component, unix.O_RDONLY|unix.O_DIRECTORY|unix.O_NOFOLLOW|unix.O_CLOEXEC, 0)
		}
		if openErr != nil {
			return closeCurrent(i18n.Errorf("打开客户端状态目录失败: %w", openErr))
		}
		var stat unix.Stat_t
		if err := unix.Fstat(next, &stat); err != nil {
			_ = unix.Close(next)
			return closeCurrent(i18n.Errorf("检查客户端状态目录失败: %w", err))
		}
		if stat.Mode&unix.S_IFMT != unix.S_IFDIR || !validClientDirectoryOwner(stat.Uid, plan.uid, os.Geteuid(), created) {
			_ = unix.Close(next)
			return closeCurrent(i18n.Errorf("客户端状态目录所有权无效"))
		}
		finalDirectory := index == len(plan.directoryComponents)-1
		if prepare && created {
			if err := unix.Fchown(next, plan.uid, plan.gid); err != nil {
				_ = unix.Close(next)
				return closeCurrent(i18n.Errorf("设置客户端状态目录所有者失败: %w", err))
			}
		}
		if prepare && (created || finalDirectory) {
			if err := unix.Fchmod(next, 0o700); err != nil {
				_ = unix.Close(next)
				return closeCurrent(i18n.Errorf("设置客户端状态目录权限失败: %w", err))
			}
		}
		if prepare && (created || finalDirectory) {
			if err := unix.Fsync(next); err != nil {
				_ = unix.Close(next)
				return closeCurrent(i18n.Errorf("同步客户端状态目录失败: %w", err))
			}
		}
...
func writeOwnedClientFile(plan *clientOwnerPlan, content []byte, mode os.FileMode, uid, gid int) error {
	...
	if err := unix.Fchown(fileFD, uid, gid); err != nil {
		return i18n.Errorf("设置客户端状态所有者失败: %w", err)
	}
	if err := unix.Fchmod(fileFD, uint32(mode.Perm())); err != nil {
		return i18n.Errorf("设置客户端状态权限失败: %w", err)
	}
	file = os.NewFile(uintptr(fileFD), temporaryName)
	if file == nil {
		return i18n.Errorf("创建客户端状态临时文件失败")
	}
	fileFD = -1
	if _, err := file.Write(content); err != nil {
		return i18n.Errorf("写入客户端状态失败: %w", err)
	}
	if err := file.Sync(); err != nil {
		return i18n.Errorf("同步客户端状态失败: %w", err)
	}
	closeErr := file.Close()
	file = nil
	if closeErr != nil {
		return i18n.Errorf("关闭客户端状态失败: %w", closeErr)
	}
	if err := unix.Renameat(directoryFD, temporaryName, directoryFD, plan.filename); err != nil {
		return i18n.Errorf("发布客户端状态失败: %w", err)
	}
	renamed = true
	if err := unix.Fsync(directoryFD); err != nil {
		return i18n.Errorf("同步客户端状态父目录失败: %w", err)
	}
	return nil
}
...
func readLocalClientFile(path string) ([]byte, error) {
	...
	if file == nil {
		_ = unix.Close(fd)
		return nil, i18n.Errorf("打开客户端状态失败")
	}
	info, err := file.Stat()
	if err != nil {
		_ = file.Close()
		return nil, i18n.Errorf("检查客户端状态失败: %w", err)
	}
	if !info.Mode().IsRegular() {
		_ = file.Close()
		return nil, i18n.Errorf("客户端状态必须是普通文件")
	}
	stat, ok := info.Sys().(*syscall.Stat_t)
	if !ok || stat.Uid != uint32(os.Geteuid()) {
		_ = file.Close()
		return nil, i18n.Errorf("客户端状态必须归当前用户所有")
	}
	if info.Mode().Perm()&0o077 != 0 {
		_ = file.Close()
		return nil, i18n.Errorf("客户端状态权限过宽，必须禁止组和其他用户访问")
	}
	if info.Size() > maxClientStateBytes {
		_ = file.Close()
		return nil, i18n.Errorf("客户端状态文件过大")
	}
	content, readErr := io.ReadAll(io.LimitReader(file, maxClientStateBytes+1))
	closeErr := file.Close()
	if err := errors.Join(readErr, closeErr); err != nil {
		return nil, i18n.Errorf("读取客户端状态失败: %w", err)
	}
	if len(content) > maxClientStateBytes {
		return nil, i18n.Errorf("客户端状态文件过大")
	}
...
func readOwnedClientFile(plan *clientOwnerPlan) ([]byte, error) {
	...
	if err != nil {
		return nil, i18n.Errorf("打开客户端状态失败: %w", err)
	}
	var stat unix.Stat_t
	...
	if stat.Mode&unix.S_IFMT != unix.S_IFREG || stat.Uid != uint32(plan.uid) || stat.Gid != uint32(plan.gid) {
		_ = unix.Close(fileFD)
		return nil, i18n.Errorf("客户端状态必须是 sudo 用户及其主组所有的普通文件")
	}
...
func createOwnedClientTemp(directoryFD int) (string, int, error) {
	for range clientTempAttempts {
		...
		if _, err := rand.Read(nonce[:]); err != nil {
			return "", -1, i18n.Errorf("生成客户端状态临时文件名失败: %w", err)
		}
		...
		if err != nil {
			return "", -1, i18n.Errorf("创建客户端状态临时文件失败: %w", err)
		}
		return name, fd, nil
	}
	return "", -1, i18n.Errorf("无法创建唯一的客户端状态临时文件")
}
...
func captureOwnedClientFile(path string, plan *clientOwnerPlan) (fileCheckpoint, error) {
	...
	if err != nil {
		return fileCheckpoint{}, i18n.Errorf("打开客户端状态回滚点失败: %w", err)
	}
	var stat unix.Stat_t
	if err := unix.Fstat(fileFD, &stat); err != nil {
		_ = unix.Close(fileFD)
		return fileCheckpoint{}, i18n.Errorf("检查客户端状态回滚点失败: %w", err)
	}
	if stat.Mode&unix.S_IFMT != unix.S_IFREG || stat.Uid != uint32(plan.uid) || stat.Gid != uint32(plan.gid) {
		_ = unix.Close(fileFD)
		return fileCheckpoint{}, i18n.Errorf("客户端状态回滚点必须是 sudo 用户及其主组所有的普通文件")
	}
	if stat.Mode&0o077 != 0 {
		_ = unix.Close(fileFD)
		return fileCheckpoint{}, i18n.Errorf("客户端状态回滚点权限过宽")
	}
	if stat.Size > maxClientStateBytes {
		_ = unix.Close(fileFD)
		return fileCheckpoint{}, i18n.Errorf("客户端状态回滚点文件过大")
	}
	file := os.NewFile(uintptr(fileFD), plan.filename)
	if file == nil {
		_ = unix.Close(fileFD)
		return fileCheckpoint{}, i18n.Errorf("打开客户端状态回滚点失败")
	}
	content, readErr := io.ReadAll(io.LimitReader(file, maxClientStateBytes+1))
	closeErr := file.Close()
	if err := errors.Join(readErr, closeErr); err != nil {
...
func restoreOwnedClientFile(checkpoint fileCheckpoint) error {
	...
	if err := unix.Unlinkat(directoryFD, checkpoint.clientPlan.filename, 0); err != nil && !errors.Is(err, unix.ENOENT) {
		return i18n.Errorf("移除客户端状态失败: %w", err)
	}
